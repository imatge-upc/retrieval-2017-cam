...
def re_ranking(desc_query, class_list, batch_size, image_names, indices, dataset, top_n_ranking, pca_matrix, model):
    ...
    for k in range(0, n_iterations+1):
        ...
        if images_hor:
            ...
            features_h, cams_h, roi_h = extract_feat_cam(model, layer, batch_size, images_h_prep, num_cams,
                                                         class_list, roi=True)
        ...
        if images_ver:
            ...
            features_v, cams_v, roi_v = extract_feat_cam(model, layer, batch_size, images_v_prep, num_cams,
                                                         class_list, roi=True)
            print 'Time extracting features: ', time.time() - t2
